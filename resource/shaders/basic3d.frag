...
void main() {
    ...
    vec3 l[5];
    vec3 EL[5];
    vec3 oriColor = vec3(1.0);
    vec3 Kd = diffusePower/float(lightNum)*oriColor/PI;
    vec3 Ks = 6.0/float(lightNum)*oriColor*(m + 8.0)/(8.0*PI);
    vec4 p = vPositionMV;
    ...
    for(int k = 0; k < lightNum; k++){
     ...
     if(useTexture == 0){
        ...
        finalColor = texture(texture_diffuse1, vTexCoord);
     }
